...
def get_theme_list(session=None):
    ...
    close_session_at_end = not session
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_theme = {
                executor.submit(get_stock_list, theme['no'], theme['name'], session=_session): theme 
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_theme), total=len(all_themes), desc=desc):
                try:
                    ...
                    logging.error(f"{theme['name']} 테마의 종목 수집 중 오류 발생: {exc}")
