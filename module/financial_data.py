...
def get_financial_data(stock_code, session=None):
    ...
    for key in response_data:
        if key['title'] == '매출액':
            ...
            revenue_data = sorted(revenue_data.items(), key=lambda x: x[0], reverse=True)[1:5]
        ...
        if key['title'] == '당기순이익':
            ...
            net_income_data = sorted(net_income_data.items(), key=lambda x: x[0], reverse=True)[1:5]
        ...
        if key['title'] == '부채비율':
            ...
            debt_ratio_data = sorted(debt_ratio_data.items(), key=lambda x: x[0], reverse=True)[1:5]
        ...
        if key['title'] == 'ROE':
            ...
            roe_data = sorted(roe_data.items(), key=lambda x: x[0],reverse=True)[1:5]
            data[f'ROE(%)'] = float(parse_num_value(roe_data[0][1]['value']))
            continue

        if key['title'] == 'PBR':
            pbr_data = key['columns']
            pbr_data = sorted(pbr_data.items(), key=lambda x: x[0],reverse=True)[1:5]
            data[f'PBR(배)'] = float(parse_num_value(pbr_data[0][1]['value']))
            continue
                            
        if key['title'] == 'PER':
            per_data = key['columns']
            per_data = sorted(per_data.items(), key=lambda x: x[0],reverse=True)[1:5]
            data[f'PER(배)'] = float(parse_num_value(per_data[0][1]['value']))
    return data
    ...
    if response.status_code == 200:
        ...
        for key in response_data:
            if key['name'] == '영업이익증가율':
                ...
                for idx, value in enumerate(reversed(profit_data)):
                    if(idx == 0):
                        continue
                    if idx == 5:
                        break
